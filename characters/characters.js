/**
 * 人物系统集成文件 - 将所有人物集成到游戏中
 * 这个文件包含了所有必要的人物代码，可以直接在game.js中使用
 */
...
CharacterManager.prototype.initializeCharacters = function() {
    ...
    for (var i = 0; i < characterConfigs.length; i++) {
        ...
        this.characters[config.id] = new BaseCharacter(config);
    }
};
...
CharacterManager.prototype.getCharacterList = function() {
    var list = [];
    for (var id in this.characters) {
        list.push({
            id: parseInt(id),
            name: this.characters[id].name,
            description: this.characters[id].description
        });
    }
